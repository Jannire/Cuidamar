...
const Home = (props) => {
    ...
    const [listadoAfectacion, setListadoAfectacion] = useState([])
    ...
    const obtenerAfectacion = async (ContaminanteID = null) => {
        const ruta = ContaminanteID == null ?
            `${RUTA_BACKEND}/Afecta2` :
            `${RUTA_BACKEND}/Afecta2?ContaminanteID=${ContaminanteID}`
        const resp = await fetch(ruta)
        const data = await resp.json()

        setListadoAfectacion(data)
    }
                                ...
                                listadoContaminantes.map((contaminantes, index) => {
                                    ...
                                    if (contaminantes.Profundidad === profu.valueOf()) {
                                        return <div className="col">
                                            ...
                                            <img id="imgContaminante" data-bs-toggle="modal" data-bs-target={`#staticBackdrop${contaminantes.ContaminanteID}`} src={base64 == "" ? "../svgs/no-image.svg" : `data:image/jpeg;base64,${base64}`} />

                                            <div className="modal fade" id={`staticBackdrop${contaminantes.ContaminanteID}`} data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                                <div className="modal-dialog">
                                                    <div className="modal-content">
                                                        <div className="modal-header">
                                                            <h1 className="modal-title fs-5 mt-1" id="staticBackdropLabel" style={{ marginRight: "15px", fontSize: "20px" }}>{`${contaminantes.Nombre}`}</h1>
                                                           

                                                            <button type="button" data-bs-dismiss="modal" className="btn-close" aria-label="Close"></button>
                                                        </div>
                                                        <div className="modal-body">
                                                            <div className="mb-3"><img id="imagenDetalle" data-bs-toggle="modal" data-bs-target={`#staticBackdrop${contaminantes.ContaminanteID}`} src={base64 == "" ? "../svgs/no-image.svg" : `data:image/jpeg;base64,${base64}`} /></div>
                                                            <div style={{ fontSize: "17px", marginBottom: "10px", marginLeft: "7px", marginRight: "7px" }}><span style={{ fontWeight: "bold" }}>Veces reportado: </span>{`${contaminantes.Contador+1}`}</div>
                                                            <div style={{ fontSize: "17px", marginBottom: "10px", marginLeft: "7px", marginRight: "7px" }}><span style={{ fontWeight: "bold" }}>Profundidad: </span>{`${contaminantes.Profundidad}m`}</div>
                                                            <div style={{ fontSize: "17px", fontWeight: "bold", marginLeft: "7px", marginRight: "7px" }}>Descripcion: </div>
                                                            <div style={{ fontSize: "17px", textAlign: "justify", marginLeft: "7px", marginRight: "7px" }}>{`${contaminantes.Descripcion}`}</div>
                                                            <div style={{ fontSize: "17px", fontWeight: "bold", marginLeft: "7px", marginRight: "7px" }}>Principales Animales afectados: </div>
                                                            {listadoAfectacion.map((afectado)=>{
                                                                if(afectado.ContaminanteID === contaminantes.ContaminanteID){
                                                                    return listadoAnimal.map((animal)=>{
                                                                        if(afectado.AnimalID===animal.AnimalID){
                                                                            return <div style={{marginLeft:"7px"}} className="row">{`- ${animal.Nombre}`}</div>
                                                                        }
                                                                    })
                                                                }
                                                            })}
